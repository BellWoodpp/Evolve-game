...
export function challengeIcon(){
    ...
    const planetTarget = $('#topBarPlanet');
    if (!planetTarget.length){
        return;
    }
    const existingFlair = planetTarget.siblings('.flair');
    if (existingFlair.length){
        clearElement(existingFlair,true);
    }
    let egg = easterEgg(1,14);
    if (egg.length > 0){
        planetTarget.after($(egg));
    }
    if (a_level > 1){
        if (egg.length === 0){
            let bIcon = getBaseIcon('topbar','challenge');
            planetTarget.after(`<span class="flair"><svg class="star${a_level}" version="1.1" x="0px" y="0px" width="16px" height="16px" viewBox="${svgViewBox(bIcon)}" xml:space="preserve">${svgIcons(bIcon)}</svg></span>`);
        }
        ...
        const flairTarget = planetTarget.siblings('.flair').first();
        if (desc.length > 0 && flairTarget.length){
            flairTarget.append($(`<div class="is-sr-only"><div>Active Challenge Genes</div>${desc}</div>`));
        }

        if (flairTarget.length){
