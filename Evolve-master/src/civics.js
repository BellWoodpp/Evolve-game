import { global, save, seededRandom, keyMultiplier, sizeApproximation, p_on, isMainTabActive } from './vars.js';
...
import { jobScale, triggerGarrisonAttackAnimation } from './jobs.js';
...
export function defineGovernment(define){
    ...
    if (!global.settings.tabLoad){
        if (!(isMainTabActive('civic') || global.settings.civicEmbedded) || global.settings.govTabs !== 0){
            return;
        }
    }

    var govern = $('<div id="government" class="government is-child"></div>');
    var governPanel = $('<div id="governmentFloatingPanel" class="government-floating-panel"></div>');
    governPanel.append(govern);
    ...
    dockGovernmentBelowFoundry();
    if (!global.settings?.governmentFloatingOpen){
        governPanel.hide();
    }
}

export function dockGovernmentBelowFoundry(){
    const panel = $('#governmentFloatingPanel');
    if (!panel.length){
        return;
    }
    ...
    panel.detach();
    panel.removeClass('government-floating is-dragging');
    panel.css({
        left: '',
        top: '',
        right: '',
    ...
    queueColumn.addClass('has-civic-government');
}

const governmentFloatingKey = 'evolve.governmentPanelPos';

function applyGovernmentFloatingPosition(panel, left, top){
    if (!panel.length){
        return { left, top };
    }
...
function restoreGovernmentFloatingPosition(panel){
    if (!panel.length || !(save && save.getItem)){
        return false;
    }
    const raw = save.getItem(governmentFloatingKey);
    if (!raw){
        return false;
    }
    try {
        ...
        if (Number.isFinite(parsed?.x) && Number.isFinite(parsed?.y)){
            applyGovernmentFloatingPosition(panel, parsed.x, parsed.y);
            return true;
        }
    }
    catch (err){
...
export function setupGovernmentFloatingPanel(){
    const panel = $('#governmentFloatingPanel');
    if (!panel.length){
        return;
    }
    ...
    if (!panel.data('floatingInitialized')){
        ...
        const restored = restoreGovernmentFloatingPosition(panel);
        if (restored){
            panel.data('floatingPositionPinned', true);
        }
        if (!restored && rect.width > 0 && rect.height > 0){
            applyGovernmentFloatingPosition(panel, rect.left, rect.top);
        }
        panel.data('floatingInitialized', true);
    }

    const doc = $(document);
    if (doc.data('governmentFloatingDragBound')){
        return;
    }
    doc.data('governmentFloatingDragBound', true);
    ...
    const stopDrag = () => {
        ...
        panel.data('floatingPositionPinned', true);
        $(document).off('pointermove.governmentDrag pointerup.governmentDrag pointercancel.governmentDrag');
        if (panel.length && save && save.setItem){
            const rect = panel[0].getBoundingClientRect();
            save.setItem(governmentFloatingKey, JSON.stringify({ x: Math.round(rect.left), y: Math.round(rect.top) }));
        }
    };
    ...
    const moveDrag = (event) => {
        ...
        applyGovernmentFloatingPosition(panel, startLeft + (event.clientX - startX), startTop + (event.clientY - startY));
    };

    doc.on('pointerdown.governmentDrag', '#governmentFloatingHeader', (event) => {
        ...
        $(document).on('pointermove.governmentDrag', moveDrag);
        $(document).on('pointerup.governmentDrag pointercancel.governmentDrag', stopDrag);
        event.preventDefault();
        event.stopPropagation();
    });

    $(window).on('resize.governmentDrag', () => {
        const panel = $('#governmentFloatingPanel');
        if (!panel.length){
            return;
        }
...
const militaryFloatingKey = 'evolve.militaryPanelPos';

function applyMilitaryFloatingPosition(panel, left, top){
    if (!panel.length){
        return { left, top };
    }
    const rect = panel[0].getBoundingClientRect();
...
function restoreMilitaryFloatingPosition(panel){
    if (!panel.length || !(save && save.getItem)){
        return false;
    }
    const raw = save.getItem(militaryFloatingKey);
    if (!raw){
        return false;
    }
    try {
        const parsed = JSON.parse(raw);
        if (Number.isFinite(parsed?.x) && Number.isFinite(parsed?.y)){
            applyMilitaryFloatingPosition(panel, parsed.x, parsed.y);
            return true;
        }
    }
    catch (err){
...
function setupMilitaryFloatingPanel(){
    const panel = $('#militaryFloatingPanel');
    if (!panel.length){
        return;
    }
    ...
    if (!panel.data('floatingInitialized')){
        ...
        if (!restoreMilitaryFloatingPosition(panel) && rect.width > 0 && rect.height > 0){
            applyMilitaryFloatingPosition(panel, rect.left, rect.top);
        }
        panel.data('floatingInitialized', true);
    }

    const doc = $(document);
    if (doc.data('militaryFloatingDragBound')){
        return;
    }
    doc.data('militaryFloatingDragBound', true);
    ...
    const stopDrag = () => {
        ...
        const panel = $('#militaryFloatingPanel');
        panel.removeClass('is-dragging');
        $(document).off('pointermove.militaryDrag pointerup.militaryDrag pointercancel.militaryDrag');
        if (panel.length && save && save.setItem){
            const rect = panel[0].getBoundingClientRect();
            save.setItem(militaryFloatingKey, JSON.stringify({ x: Math.round(rect.left), y: Math.round(rect.top) }));
        }
    };
    ...
    const moveDrag = (event) => {
        ...
        applyMilitaryFloatingPosition(panel, startLeft + (event.clientX - startX), startTop + (event.clientY - startY));
    };

    doc.on('pointerdown.militaryDrag', '#militaryFloatingHeader', (event) => {
        if (event.button !== undefined && event.button !== 0){
            return;
        }
        const panel = $('#militaryFloatingPanel');
        ...
        $(document).on('pointermove.militaryDrag', moveDrag);
        $(document).on('pointerup.militaryDrag pointercancel.militaryDrag', stopDrag);
        event.preventDefault();
        event.stopPropagation();
    });

    $(window).on('resize.militaryDrag', () => {
        const panel = $('#militaryFloatingPanel');
        if (!panel.length){
            return;
        }
...
export function defineGarrison(force){
    commisionGarrison();

    if (!global.settings.tabLoad && !force){
        if (!(isMainTabActive('civic') || global.settings.civicEmbedded) || global.settings.govTabs !== 3){
            return;
        }
    }
    ...
    const existingPanel = $('#militaryFloatingPanel');
    if (existingPanel.length && existingPanel.parent().is(military)){
        existingPanel.detach();
        clearElement(military);
        military.append(existingPanel);
    }
    else {
        clearElement(military);
    }
    const useFloatingPanel = true;
    let contentRoot = military;
    if (useFloatingPanel){
        let panel = $('#militaryFloatingPanel');
        if (!panel.length){
            panel = $('<div id="militaryFloatingPanel" class="military-floating-panel"></div>');
            panel.append(`<div id="militaryFloatingHeader" class="military-floating-header" aria-label="${loc('tab_military')}"></div>`);
            panel.append('<div id="militaryFloatingContent" class="military-floating-content"></div>');
            military.append(panel);
        }
        else if (!global.settings?.militaryFloatingOpen && !panel.parent().is(military)){
            panel.detach().appendTo(military);
        }
        if (!panel.find('#militaryFloatingContent').length){
            panel.append('<div id="militaryFloatingContent" class="military-floating-content"></div>');
        }
        contentRoot = panel.find('#militaryFloatingContent');
    }

    clearElement(contentRoot);
    var garrison = $('<div id="garrison" v-show="vis()" class="garrison tile is-child"></div>');
    contentRoot.append(garrison);
    contentRoot.append($(`<div id="fortress"></div>`));
    
    buildGarrison(garrison,true);
    defineMad(contentRoot);
    if (useFloatingPanel){
        setupMilitaryFloatingPanel();
    }
}
        ...
        methods: {
            trigModal(){
                ...
                var checkExist = setInterval(function() {
                   if ($('.modal.is-active #modalBox').length > 0) {
...
function drawGovModal(){
    const modalBox = $('.modal.is-active #modalBox').last();
    if (!modalBox.length){
        return;
    }
    modalBox.empty();
    const title = $(`<p id="modalBoxTitle" class="has-text-warning modalTitle">${loc('civics_government_type')}</p>`);
...
export function foreignGov(){
    if ($('#foreign').length === 0 && !global.race['cataclysm'] && (!global.tech['world_control'] || global.race['truepath']) && !global.tech['isolation']){
        ...
        for (let i=0;i<govEnd;i++){
            ...
            let actions = $(`<div></div>`);
            actions.append($(`<button :label="battleAssessment(${i})" class="button gaction attack" @click="campaign(${i})"><span class="gaction-attack-icon" v-show="!f${i}.occ && !f${i}.anx && !f${i}.buy"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-swords-icon lucide-swords"><polyline points="14.5 17.5 3 6 3 3 6 3 17.5 14.5"/><line x1="13" x2="19" y1="19" y2="13"/><line x1="16" x2="20" y1="16" y2="20"/><line x1="19" x2="21" y1="21" y2="19"/><polyline points="14.5 6.5 18 3 21 3 21 6 17.5 9.5"/><line x1="5" x2="9" y1="14" y2="18"/><line x1="7" x2="4" y1="17" y2="20"/><line x1="3" x2="5" y1="19" y2="21"/></svg><span class="is-sr-only">${loc('civics_garrison_attack')}</span></span><span v-show="f${i}.occ || f${i}.anx || f${i}.buy">${loc('civics_garrison_unoccupy')}</span></button>`));
                ...
                function(){ return '<span>{{ label() }}</span>'; },
                {
                    ...
                    in: function(obj){
                        vBind({
                            ...
                            methods: {
                                label(){
                                    return battleAssessmentLabel(i, loc('civics_garrison_attack'));
                                }
                            }
                        });
                    },
    ...
    if ((!global.tech['world_control'] || global.race['truepath']) && !global.race['cataclysm'] && !global.tech['isolation']){
        ...
        if (full){
            if (global.race['truepath'] && global.tech['rival']){
                campaign.append($(`<div class="launch gov3" v-show="rvis()"><div class="has-text-caution">${govTitle(3)}</div><button class="button campaign" @click="campaign(3)"><span class="campaign-launch-icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-sword-icon lucide-sword"><path d="m11 19-6-6"/><path d="m5 21-2-2"/><path d="m8 16-4 4"/><path d="M9.5 17.5 21 6V3h-3L6.5 14.5"/></svg><span class="is-sr-only">${loc('civics_garrison_launch_campaign')}</span></span></button></div>`));
            }
            if (!global.tech['world_control']){
                campaign.append($(`<div class="launch gov0"><div class="has-text-caution">${govTitle(0)}</div><button class="button campaign" @click="campaign(0)"><span class="campaign-launch-icon" v-show="!g0.occ && !g0.anx && !g0.buy"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-sword-icon lucide-sword"><path d="m11 19-6-6"/><path d="m5 21-2-2"/><path d="m8 16-4 4"/><path d="M9.5 17.5 21 6V3h-3L6.5 14.5"/></svg><span class="is-sr-only">${loc('civics_garrison_launch_campaign')}</span></span><span v-show="g0.occ || g0.anx || g0.buy">${loc('civics_garrison_deoccupy')}</span></button></div>`));
                campaign.append($(`<div class="launch gov1"><div class="has-text-caution">${govTitle(1)}</div><button class="button campaign" @click="campaign(1)"><span class="campaign-launch-icon" v-show="!g1.occ && !g1.anx && !g1.buy"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-sword-icon lucide-sword"><path d="m11 19-6-6"/><path d="m5 21-2-2"/><path d="m8 16-4 4"/><path d="M9.5 17.5 21 6V3h-3L6.5 14.5"/></svg><span class="is-sr-only">${loc('civics_garrison_launch_campaign')}</span></span><span v-show="g1.occ || g1.anx || g1.buy">${loc('civics_garrison_deoccupy')}</span></button></div>`));
                campaign.append($(`<div class="launch gov2"><div class="has-text-caution">${govTitle(2)}</div><button class="button campaign" @click="campaign(2)"><span class="campaign-launch-icon" v-show="!g2.occ && !g2.anx && !g2.buy"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-sword-icon lucide-sword"><path d="m11 19-6-6"/><path d="m5 21-2-2"/><path d="m8 16-4 4"/><path d="M9.5 17.5 21 6V3h-3L6.5 14.5"/></svg><span class="is-sr-only">${loc('civics_garrison_launch_campaign')}</span></span><span v-show="g2.occ || g2.anx || g2.buy">${loc('civics_garrison_deoccupy')}</span></button></div>`));
            }
        }
    }
                ...
                function(){ return '<span>{{ label() }}</span>'; },
                {
                    ...
                    in: function(obj){
                        vBind({
                            ...
                            methods: {
                                label(){
                                    return battleAssessmentLabel(i, loc('civics_garrison_launch_campaign'));
                                }
                            }
                        });
                    },
...
function battleAssessmentLabel(gov, prefix){
    const state = global.civic.foreign[`gov${gov}`];
    if (state?.occ || state?.buy || state?.anx){
        return battleAssessment(gov);
    }
    return `${prefix}: ${battleAssessment(gov)}`;
}
...
function war_campaign(gov){
    ...
    triggerGarrisonAttackAnimation(global.civic.garrison.raid);
...
function defineMad(container){
    if (global.race['sludge'] || global.race['ultra_sludge']){ return false; }
    const root = container && container.length ? container : $('#military');
    if (!root.length){
        return false;
    }
    const existing = $('#mad');
    if (existing.length){
        if (!existing.parent().is(root)){
            existing.detach().appendTo(root);
        }
        return true;
    }
    if ($(`#mad`).length === 0){
