...
if (convertVersion(global['version']) < 9000){
    if (global.settings){
        ...
        else if (!global.settings.hasOwnProperty('showCity') && global.settings.showCiv){
            global.settings.showCity = global.settings.showCiv;
        }
    }
}
...
if (!global.settings.hasOwnProperty('researchUnlocked')){
    const woodAmount = global.resource?.Lumber?.amount ?? 0;
    const stoneAmount = global.resource?.Stone?.amount ?? 0;
    global.settings.researchUnlocked = woodAmount >= 6 || stoneAmount >= 6;
}
if (!global.settings.researchUnlocked){
    global.settings.showResearch = false;
}
else if (!global.settings.showResearch){
    global.settings.showResearch = true;
}

if (!global.settings.hasOwnProperty('resourcesFloating')){
    global.settings.resourcesFloating = false;
}
if (!global.settings.hasOwnProperty('hideResourcesPanel')){
    global.settings.hideResourcesPanel = false;
}

if (!global.settings.hasOwnProperty('leftColumnCollapsed')){
    global.settings.leftColumnCollapsed = false;
}

if (!global.settings.hasOwnProperty('resourcePanelFloating')){
...
if (typeof global.settings.civicEmbedded === 'undefined'){
...
if (global.city?.mine?.count > 0 && global.resource?.Copper && !global.resource.Copper.display){
    global.resource.Copper.display = true;
}
if (global.city?.coal_mine?.count > 0 && global.resource?.Coal && !global.resource.Coal.display){
...
export function resizeGame(){
    ...
    const mainColumn = $('#mainColumn');
    const sideQueue = $('#sideQueue');
    ...
    if (!queueColumn.length || !mainColumn.length){
        return;
    }

    const queueFloating = queue.hasClass('msgQueue-floating');
    const queueEmbedded = queue.hasClass('msgQueue-embedded');
    let queueDockedRight = queue.hasClass('right');
    const forceFloatingOutsideLeft = global.race?.species === 'protoplasm';
    const ensureMsgQueuePanelVisible = () => {
        const panel = $('#msgQueuePanel');
        if (!panel.length){
            return;
        }
        const panelPosition = panel.css('position');
        ...
        const isOnScreen = hasSize
            && rect.bottom > 0
            && rect.right > 0
            && rect.left < viewportWidth
            && rect.top < viewportHeight;
        if (!isOnScreen){
            panel.css({ left: '0.5rem', bottom: '0.5rem', right: 'auto', top: 'auto' });
            if (save && save.removeItem){
                save.removeItem('evolve.msgQueuePanelPos');
            }
            return;
        }
        ...
        if (nextLeft !== rect.left || nextTop !== rect.top){
            panel.css({ left: `${nextLeft}px`, top: `${nextTop}px`, right: 'auto', bottom: 'auto' });
            if (save && save.setItem){
                save.setItem('evolve.msgQueuePanelPos', JSON.stringify({ x: Math.round(nextLeft), y: Math.round(nextTop) }));
            }
        }
    };
    ...
    if (queueEmbedded){
        if (resourcesPanel.length){
            const resourcesParent = resourcesPanel.parent()[0];
            const shouldMove = !queue.parent().length
                || queue.parent()[0] !== resourcesParent
                || !queue.prev('#resources').length;
            if (shouldMove){
                queue.detach();
                resourcesPanel.after(queue);
            }
        }
        else if (leftColumn.length && (!queue.parent().length || queue.parent()[0] !== leftColumn[0])){
            queue.detach().appendTo(leftColumn);
        }
        if (queueDockedRight){
            queue.removeClass('right');
            queueDockedRight = false;
        }
        queue.addClass('has-text-info');
        queue.css('resize', '');
    }
    else if (!queueFloating && !queueDockedRight){
        queue.detach();
        queue.addClass('right');
        queue.removeClass('has-text-info');
        queue.css('resize', 'none');
        queueColumn.append(queue);
        queueDockedRight = true;
    } else if (queueFloating){
        if (queueDockedRight){
            queue.removeClass('right sticky');
            queue.addClass('vscroll');
            queueDockedRight = false;
        }
        if (forceFloatingOutsideLeft){
            const floatingRoot = mainRoot.length ? mainRoot : $('body');
            if (!queue.parent().length || queue.parent()[0] !== floatingRoot[0]){
                queue.detach().appendTo(floatingRoot);
            }
            ensureMsgQueuePanelVisible();
        }
        else if (sideQueue.length && (!queue.parent().length || queue.parent()[0] !== sideQueue[0])){
            queue.detach().appendTo(sideQueue);
        }
        queue.addClass('has-text-info');
        queue.css('resize', '');
    }

    if (queueColumn.hasClass('is-hidden')){
        queueColumn.removeClass('is-hidden');
    }
    const queueColumnActive = queueColumn.children(':visible')
        .not('#civic-military-panel.civic-military-floating, #governmentFloatingPanel.government-floating')
        .length > 0;
    const useRightColumn = queueColumnActive;
    if (useRightColumn){
        queueColumn.removeClass('is-hidden');
        queueColumn.addClass('is-one-quarter');
        mainColumn.removeClass('is-half is-full');
        mainColumn.addClass('is-three-quarters');
    }
    else {
        queueColumn.removeClass('is-one-quarter');
        queueColumn.addClass('is-hidden');
        mainColumn.removeClass('is-half is-three-quarters');
        mainColumn.addClass('is-full');
    }
}
...
export function getMainTabKeys(){
    const settings = global.settings || {};
    const tabs = [];
    if (settings.showEvolve){
        tabs.push('evolution');
    }
    if (settings.showCiv){
        tabs.push('civ');
    }
    if (settings.showCivic){
        tabs.push('civic');
    }
    if (settings.showResearch && !settings.researchFloatingOnly){
        tabs.push('research');
    }
    if (settings.showResources){
        tabs.push('resources');
    }
    if (settings.showGenetics){
        tabs.push('arpa');
    }
    if (settings.showAchieve){
        tabs.push('stats');
    }
    return tabs;
}

export function getActiveMainTabKey(tabIndex){
    const keys = getMainTabKeys();
    if (!keys.length){
        return null;
    }
    const index = Number.isFinite(tabIndex)
        ? tabIndex
        : (Number.isFinite(global.settings?.civTabs) ? global.settings.civTabs : 0);
...
export function isMainTabActive(tabKey, tabIndex){
    return getActiveMainTabKey(tabIndex) === tabKey;
}

export function getMainTabIndex(tabKey){
    if (!tabKey){
        return -1;
    }
    return getMainTabKeys().indexOf(tabKey);
}
