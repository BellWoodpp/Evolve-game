import { global, seededRandom, save, webWorker, power_generated, keyMultiplier, sizeApproximation, active_rituals, isMainTabActive } from './vars.js';
...
export function renderSupernatural(){
    if (!global.settings.tabLoad){
        if (!(isMainTabActive('civic') || global.settings.civicEmbedded) || global.settings.govTabs !== 7){
            return;
        }
    }
...
export function renderPsychicPowers(){
    if (!global.settings.tabLoad){
        if (!(isMainTabActive('civic') || global.settings.civicEmbedded) || global.settings.govTabs !== 6){
            return;
        }
    }
...
// 种族选择器函数
export function getBasicRaces(){
    let basicList = Object.keys(races).filter(function(r){
        return !['custom','hybrid'].includes(r) && !['protoplasm','hellspawn','junker'].includes(r);
    });
    return basicList;
}

export function randomRaceSelector(){
    const basicList = getBasicRaces();
    return basicList[Math.floor(Math.random() * basicList.length)];
}

export function selectRace(raceName){
    if (global.race.species === raceName){
        return;
    }

    // 重置游戏到新的种族
    let oldRace = global.race.species;
    global.race.species = raceName;

    // 重新初始化资源
    if (races[raceName].type === 'organism'){
        global.race.species = raceName;  // protoplasm
    }
    else if (races[raceName].type === 'humanoid'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'carnivore'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'herbivore'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'omnivore'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'small'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'giant'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'reptilian'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'avian'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'insectoid'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'plant'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'fungi'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'aquatic'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'fey'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'heat'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'polar'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'sand'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'demonic'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'angelic'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'synthetic'){
        global.race.species = raceName;
    }
    else if (races[raceName].type === 'eldritch'){
        global.race.species = raceName;
    }
