import { global, p_on, support_on, sizeApproximation, keyMap, isMainTabActive, getMainTabIndex } from './vars.js';
...
export function renderTauCeti(){
    if (!global.settings.tabLoad && (!isMainTabActive('civ') || global.settings.spaceTabs !== 6)){
        return;
    }
    let parent = $('#tauceti');
    clearElement(parent);
...
export function drawShipYard(){
    if (!global.settings.tabLoad){
        if (!(isMainTabActive('civic') || global.settings.civicEmbedded) || global.settings.govTabs !== 5){
            return;
        }
    }
    setOrbits();
