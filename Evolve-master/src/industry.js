import { global, keyMultiplier, sizeApproximation, p_on, support_on, quantum_level, callback_queue, active_rituals, isMainTabActive } from './vars.js';
...
export function defineIndustry(){
    if (!global.settings.tabLoad){
        if (!(isMainTabActive('civic') || global.settings.civicEmbedded) || global.settings.govTabs !== 1){
            return;
        }
    }
...
export function setPowerGrid(){
    const floatingOpen = Boolean(global.settings?.powerGridFloatingOpen);
    if (!global.settings.tabLoad){
        const civicActive = isMainTabActive('civic') || global.settings.civicEmbedded;
        if (!(civicActive && global.settings.govTabs === 2) && !floatingOpen){
            return;
        }
    }
    let grids = gridDefs();
    ...
    const floatingRoot = $('#powerGridFloatingContent');
    const powerRoot = floatingOpen && floatingRoot.length ? floatingRoot : $('#powerGrid');
    if (!powerRoot.length){
        return;
    }
    if (floatingOpen && floatingRoot.length){
        const inlineRoot = $('#powerGrid');
        if (inlineRoot.length && inlineRoot[0] !== powerRoot[0]){
            clearElement(inlineRoot);
        }
    }
    clearElement(powerRoot);
    powerRoot.append(`<div class="powerGridHead"><div class="powerGridHeader has-text-info">${loc(`power_grid_header`)}</div><div id="powerModeSwitch"><b-switch class="setting" v-model="lowPowerBalance">Distribute Low Power</b-switch></div></div>`);
    vBind({
        el: `#powerModeSwitch`,
    ...
    Object.keys(grids).forEach(function(grid_type){
        ...
        if (grids[grid_type].r && grids[grid_type].rs && global[grids[grid_type].r][grids[grid_type].rs]){
            powerRoot.append(`<div id="pg${grid_type}sup" class="gridHeader"><span class="has-text-caution">${grids[grid_type].n}</span> {{ support }}/{{ s_max }}</div>`);
            vBind({
                el: `#pg${grid_type}sup`,
            ...
            powerRoot.append(`<div class="gridHeader has-text-caution">${grids[grid_type].n}${candy}</div>`);
        }

        let grid = $(`<div id="grid${grid_type}" class="powerGrid"></div>`);
        powerRoot.append(grid);
