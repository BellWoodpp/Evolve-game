...
import { actions, actionDesc, checkAffordable, drawTech, resQueue } from './actions.js';
...
export function popover(id,content,opts){
    ...
    if (!opts.hasOwnProperty('strategy')){ opts['strategy'] = 'absolute'; }
    if (opts['bind']){
...
export function updateQueueBadges(targetId){
    const removeQueuedById = (id, removeAll) => {
    ...
    const removeResearchQueuedById = (id, removeAll) => {
        if (!global.r_queue || !global.r_queue.queue || global.r_queue.queue.length === 0){
            return false;
        }
        let removed = false;
        for (let i=global.r_queue.queue.length - 1; i>=0; i--){
            let item = global.r_queue.queue[i];
            if (item.id !== id){
                continue;
            }
        ...
        if (removed){
            if (typeof resQueue === 'function'){
                resQueue();
            }
            if (typeof drawTech === 'function'){
                drawTech();
            }
        }
        return removed;
    };
    const resolveQueueAction = (item) => {
        if (!item || !item.action){
            return null;
        }
        if (item.action === 'tp-ship' || item.action === 'hell-mech'){
            return null;
        }
        if (item.action === 'arpa'){
            return actions.arpa?.[item.type] ?? null;
        }
        const deepScan = ['space','interstellar','galaxy','portal','tauceti','eden'];
        if (deepScan.includes(item.action)){
            for (let region in actions[item.action]){
                if (actions[item.action][region]?.[item.type]){
                    return actions[item.action][region][item.type];
                }
            }
            return null;
        }
        return actions[item.action]?.[item.type] ?? null;
    };
    const resolveQueueTime = (item) => {
        if (item?.cna || item?.time < 0){
            return -1;
        }
        if (typeof item?.time === 'number' && item.time > 0){
            return item.time;
        }
        const c_action = resolveQueueAction(item);
        if (c_action && !checkAffordable(c_action, true, c_action['doNotAdjustCost'] ? true : false)){
            return -1;
        }
        if (typeof item?.time === 'number' && item.time === 0){
            if (c_action?.cost){
                let t = timeCheck(c_action, false);
                return t < 0 ? -1 : t;
            }
        }
        return typeof item?.time === 'number' ? item.time : null;
    };
    const resolveResearchTime = (item) => {
        if (!item){
            return null;
        }
        if (item.cna || item.time < 0){
    ...
    const clearActionBadge = (action) => {
        if (!action){
            return;
        }
        action.classList.remove('queueing');
        action.querySelectorAll('.queue-badge').forEach((badge) => badge.remove());
    };
    ...
    const attachBadgeHandler = (action) => {
        if (!action || action.__queueBadgeHandler){
            return;
        }
        const handler = (event) => {
            const state = action.__queueBadgeState;
            if (!state || typeof state.removeFn !== 'function'){
                return;
            }
            const badge = action.querySelector('.queue-badge');
            if (!badge){
                return;
            }
            ...
            const refresh = state.removeFn(state.id, event.shiftKey);
            if (refresh !== false){
                updateQueueBadges();
            }
        };
        action.__queueBadgeHandler = handler;
        action.addEventListener('pointerdown', handler, true);
        action.addEventListener('click', handler, true);
        action.addEventListener('touchstart', handler, { capture: true, passive: false });
    };
    const attachResearchBadgeHandler = (badge, id, removeFn) => {
        if (!badge || badge.__queueBadgeHandler){
            return;
        }
        const handler = (event) => {
            event.preventDefault();
            ...
            const refresh = removeFn(id, event.shiftKey);
            if (refresh !== false){
                updateQueueBadges();
            }
        };
        badge.__queueBadgeHandler = handler;
        badge.addEventListener('pointerdown', handler, true);
        badge.addEventListener('click', handler, true);
        badge.addEventListener('touchstart', handler, { capture: true, passive: false });
    };
    const addBadge = (action, id, posList, timeVal, removeFn, queueType) => {
        if (!action || !posList || !posList.length){
            return;
        }
        action.classList.add('queueing');
        const badge = document.createElement('span');
        badge.className = 'queue-badge';
        if (queueType){
            badge.dataset.queueType = queueType;
        }
        badge.setAttribute('role', 'button');
        badge.setAttribute('aria-label', loc('queue_remove'));
        const posSpan = document.createElement('span');
        posSpan.className = 'queue-badge-positions';
        posSpan.textContent = posList.join('.');
        badge.append(posSpan);
        if (typeof timeVal === 'number'){
            const timeSpan = document.createElement('span');
            const isNever = timeVal < 0;
            timeSpan.className = `queue-badge-time ${isNever ? 'queue-badge-time--never' : 'queue-badge-time--ready'}`;
            timeSpan.textContent = isNever ? loc('time_never') : timeFormat(timeVal);
            badge.append(timeSpan);
        }
        if (typeof removeFn === 'function'){
            if (queueType === 'research'){
                attachResearchBadgeHandler(badge, id, removeFn);
            }
            else {
                attachBadgeHandler(action);
                action.__queueBadgeState = { id, removeFn };
            }
        }
        action.prepend(badge);
    };

    const hasBuildQueue = !!(global.tech['queue'] && global.queue && global.queue.queue && global.queue.queue.length);
    const hasResearchQueue = !!(global.tech['r_queue'] && global.r_queue && global.r_queue.queue && global.r_queue.queue.length);

    if (targetId){
        const action = document.getElementById(targetId);
        clearActionBadge(action);
        if (!action){
            return;
        }
        ...
        if (hasBuildQueue){
            ...
            const times = new Map();
            let slot = 0;
            global.queue.queue.forEach((item) => {
                const step = Math.max(1, Math.ceil(item.q / item.qs));
                for (let i = 0; i < step; i++){
                    slot += 1;
                    if (!positions.has(item.id)){
                        positions.set(item.id, []);
                    }
                    positions.get(item.id).push(slot);
                }
                const timeVal = resolveQueueTime(item);
                if (typeof timeVal === 'number'){
                    const prev = times.get(item.id);
                    if (typeof prev === 'undefined' || timeVal < prev || prev >= 0 && timeVal < 0){
                        times.set(item.id, timeVal);
                    }
                }
            });
            addBadge(action, targetId, positions.get(targetId), times.get(targetId), (id, removeAll) => {
                removeQueuedById(id, removeAll);
                buildQueue();
                return false;
            });
        }
        if (hasResearchQueue){
            ...
            const times = new Map();
            let slot = 0;
            global.r_queue.queue.forEach((item) => {
                slot += 1;
                if (!positions.has(item.id)){
                    positions.set(item.id, []);
                }
                positions.get(item.id).push(slot);
                const timeVal = resolveResearchTime(item);
                if (typeof timeVal === 'number'){
                    const prev = times.get(item.id);
                    if (typeof prev === 'undefined' || timeVal < prev || prev >= 0 && timeVal < 0){
                        times.set(item.id, timeVal);
            ...
            addBadge(action, targetId, positions.get(targetId), times.get(targetId), removeResearchQueuedById, 'research');
        }
        return;
    }

    document.querySelectorAll('.queue-badge').forEach((badge) => badge.remove());
    document.querySelectorAll('.action.queueing').forEach((action) => {
        action.classList.remove('queueing');
    });
    ...
    if (hasBuildQueue){
        ...
        positions.forEach((posList, id) => {
            const action = document.getElementById(id);
            addBadge(action, id, posList, times.get(id), (queueId, removeAll) => {
                removeQueuedById(queueId, removeAll);
                buildQueue();
                return false;
            });
        });
    ...
    if (hasResearchQueue){
        ...
        positions.forEach((posList, id) => {
            const action = document.getElementById(id);
            addBadge(action, id, posList, times.get(id), removeResearchQueuedById, 'research');
        });
    }
}
