import { global, seededRandom, keyMultiplier, p_on, support_on, gal_on, spire_on, hell_reports, hell_graphs, sizeApproximation, keyMap, getActiveMainTabKey, getMainTabIndex } from './vars.js';
...
export function renderFortress(){
    if (!global.settings.tabLoad && (getActiveMainTabKey() !== 'civ' || global.settings.spaceTabs !== 4)){
        return;
    }
    let parent = $('#portal');
    clearElement(parent);
...
function buildEnemyFortress(parent){
    ...
    if (!global.settings.tabLoad){
        const activeMainTab = getActiveMainTabKey();
        if (activeMainTab === 'civ'){
            if (global.settings.spaceTabs !== 4){
                return;
            }
        }
        else if (activeMainTab === 'civic'){
...
export function drawMechLab(){
    if (!global.settings.tabLoad){
        if (!(getActiveMainTabKey() === 'civic' || global.settings.civicEmbedded) || global.settings.govTabs !== 4){
            return;
        }
    }
