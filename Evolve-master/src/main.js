import { global, save, seededRandom, webWorker, intervals, keyMap, atrack, resizeGame, breakdown, sizeApproximation, keyMultiplier, power_generated, p_on, support_on, int_on, gal_on, spire_on, set_qlevel, quantum_level, callback_queue, active_rituals, getMainTabIndex, isMainTabActive } from './vars.js';
...
import { defineJobs, ensureMoraleJobControls, job_desc, loadFoundry, farmerValue, jobName, jobScale, workerScale, limitCraftsmen, loadServants, assignDefaultCraftsmen, getDefaultPopulationJob} from './jobs.js';
...
import { actions, updateDesc, updatePopulationCounters, checkTechRequirements, drawEvolution, BHStorageMulti, storageMultipler, checkAffordable, calcAffordableCount, updateAffordableCountBadge, checkPowerRequirements, drawCity, drawTech, gainTech, housingLabel, updateQueueNames, wardenLabel, planetGeology, resQueue, bank_vault, start_cataclysm, orbitDecayed, postBuild, skipRequirement, structName, templeCount, initStruct, casino_vault, casinoEarn, doCallbacks, cLabels, preloadEvolutionActionImage } from './actions.js';
...
import { index, mainVue, initTabs, loadTab, refreshTopBarAndResearchToggle, updateResearchFloatingToggleBadge } from './index.js';
...
const dogPetSound = new Audio('/voice/dog/mixkit-dog.wav');
dogPetSound.preload = 'auto';
dogPetSound.volume = 0.7;

function shouldPlayClickSound(target){
    if (!target.length){
        return false;
    }
    if (target.is('[aria-disabled="true"], .disabled, .is-disabled')){
        return false;
    }
    if (target.closest('#settings, .settings').length){
        return false;
    }
    return true;
}
...
$(document).on('click', function(e){
    const target = $(e.target).closest('a.button, button.button, span.button, .text-button, [role="button"]');
    if (!shouldPlayClickSound(target)){
        return;
    }
    playClickSound();
});
...
$(document).keydown(function(e){
    ...
    if (!$(`input`).is(':focus') && !$(`textarea`).is(':focus')){
        const evolutionActive = isMainTabActive('evolution');
        if (key === global.settings.keyMap.settings && !evolutionActive){
            global.settings.settingsModal = true;
            return;
        }
        Object.keys(quickMap).forEach(function(k){
            if (key === global.settings.keyMap[k] && !evolutionActive && global.settings[k]){
                const targetKey = quickMap[k];
                const targetIndex = getMainTabIndex(targetKey);
                if (targetIndex < 0){
                    return;
                }
                if (global.settings.civTabs !== targetIndex) {
                    global.settings.civTabs = targetIndex;
                }
                else {
...
setTimeout(() => {
    if (window.evolveActionSfx){
        window.evolveActionSfx.ready = true;
    }
}, 1200);
...
popover('pinguiculaPortrait',
    function(){
        return loc('race_pinguicula_desc');
    },{
        elm: '#pinguiculaPortrait',
        placement: 'bottom',
        strategy: 'fixed',
        offset: [0, 6],
        attach: 'body',
        in: function(obj){
            const applyPosition = () => {
                const target = document.querySelector('#pinguiculaPortrait');
                if (!target || !obj || !obj.popper || !obj.popper.length){
                    return;
                }
                const rect = target.getBoundingClientRect();
                const popperEl = obj.popper[0];
                const popRect = popperEl.getBoundingClientRect();
                const padding = 8;
                let left = rect.left + (rect.width / 2) - (popRect.width / 2);
                let top = rect.bottom + 6;
                left = Math.min(window.innerWidth - popRect.width - padding, Math.max(padding, left));
                top = Math.min(window.innerHeight - popRect.height - padding, Math.max(padding, top));
...
vBind({
    ...
    methods: {
        ...
        petPet(){
            if (global.race?.pet?.type === 'dog'){
                try {
                    dogPetSound.currentTime = 0;
                    dogPetSound.play();
                }
                catch {
                    // Ignore playback errors (e.g. autoplay restrictions).
...
function initTopBarPlanetPopover(){
    const target = '#topBarPlanet';
    $(target).off('mouseover mouseout mouseenter mouseleave touchend');
...
function initEvolutionTabPopover(){
    const target = '#mainTabs .tabs > ul > li.main-tab-evolution';
    if (!$(target).length){
        return;
    }
    $(target).off('mouseover mouseout mouseenter mouseleave touchend');
    popover('evolutionTab', function(){
        return `<div>${loc('tab_evolve_desc')}</div>`;
    },{
        elm: target,
        placement: 'bottom',
        offset: [0, 6],
...
let civilTabPopoverRetryId = null;

function initCivilTabPopover(){
    if (civilTabPopoverRetryId){
        clearTimeout(civilTabPopoverRetryId);
        civilTabPopoverRetryId = null;
    }
    const target = '#mainTabs .tabs > ul > li.main-tab-civil';
    if (!$(target).length){
        civilTabPopoverRetryId = setTimeout(() => {
            civilTabPopoverRetryId = null;
            initCivilTabPopover();
        }, 200);
        return;
    ...
    $(target).off('mouseover mouseout mouseenter mouseleave touchend');
    popover('civilTab', function(){
        return `<div class="civ-tier-popover"><div>${loc('tab_civil_tier_i_hint')}</div><div class="civ-tier-desc">${loc('tab_civil_tier_i_desc')}</div></div>`;
    },{
        elm: target,
        placement: 'bottom',
        offset: [0, 6],
...
let pinguiculaPopCache = { amount: null, max: null, display: null };
function fastLoop(){
    ensureCoreResourceVisibility();
    ...
    if (global.race.species === 'protoplasm'){
        ...
        if (global.resource?.RNA?.amount > 0){
            preloadEvolutionActionImage('dna');
            preloadEvolutionActionImage('membrane');
        }
        if (global.resource?.DNA?.display){
            preloadEvolutionActionImage('organelles');
        }
        if (global.evolution['organelles']){
            preloadEvolutionActionImage('nucleus');
        }
        if (global.evolution['nucleus']){
            preloadEvolutionActionImage('eukaryotic_cell');
        }
        if (global.evolution['eukaryotic_cell']){
            preloadEvolutionActionImage('mitochondria');
        }
        ...
        const defaultJob = getDefaultPopulationJob();
        global.civic[defaultJob].workers += global.resource[global.race.species].amount - total;
        if (global.civic[defaultJob].workers < 0){
            global.civic[defaultJob].workers = 0;
        }
        ...
        // Detect new unlocks
        const woodAmount = global.resource?.Lumber?.amount ?? 0;
        const stoneAmount = global.resource?.Stone?.amount ?? 0;
        if (!global.settings.researchUnlocked && (woodAmount >= 6 || stoneAmount >= 6)){
    ...
    if (global.race?.species === 'pinguicula'){
        const popRes = global.resource?.[global.race.species];
        if (popRes && $(`#res${global.race.species}`).length){
            if (popRes.amount !== pinguiculaPopCache.amount || popRes.max !== pinguiculaPopCache.max || popRes.display !== pinguiculaPopCache.display){
                pinguiculaPopCache = { amount: popRes.amount, max: popRes.max, display: popRes.display };
                vBind({ el: `#res${global.race.species}` },'update');
...
function midLoop(){
    ...
    if (global.race.species === 'protoplasm'){
        ...
        Object.keys(actions.evolution).forEach(function (action){
            if (actions.evolution[action] && actions.evolution[action].cost){
                ...
                if (element.length > 0){
                    ...
                    let affordCount = calcAffordableCount(c_action, 'evolution');
                    updateAffordableCountBadge(c_action.id, affordCount);
                }
            }
        });
    }
        ...
        Object.keys(caps).forEach(function (res){
            ...
            const countEls = $(`#res${res} .count, #res${res}Outskirts .count, #res${res}Military .count, #res${res}Industrial .count, #res${res}Trade .count, #res${res}Science .count, #res${res}Residential .count, #res${res}Commercial .count`);
            if (global.resource[res].amount >= global.resource[res].max * 0.99){
                if (!countEls.hasClass('has-text-warning')){
                    countEls.addClass('has-text-warning');
                }
            }
            else if (countEls.hasClass('has-text-warning')){
                countEls.removeClass('has-text-warning');
            }
        });
        ...
        // Ensure entertainer visibility reflects actual caps (e.g., after amphitheatre builds).
        if (global.civic?.entertainer){
            const shouldShowEntertainer = lCaps['entertainer'] > 0;
            if (global.civic.entertainer.display !== shouldShowEntertainer){
                global.civic.entertainer.display = shouldShowEntertainer;
                if (typeof vBind === 'function'){
                    if ($('#moraleJobControls').length){
                        vBind({ el: '#moraleJobControls' }, 'update');
                    }
                    if ($('#civ-entertainer').length){
                        vBind({ el: '#civ-entertainer' }, 'update');
                    }
                }
            }
        }
        ...
        cityList.forEach(function (action){
            if (actions.city[action] && actions.city[action].cost){
                ...
                if (element.length > 0){
                    ...
                    let affordCount = calcAffordableCount(c_action, 'city');
                    updateAffordableCountBadge(c_action.id, affordCount);
                }
...

    {
        ...
        const msgQueueFloating = msgQueue.hasClass('msgQueue-floating');
        ...
        const resourcesDockedTop = resources.hasClass('resources-top');
        const resourcesDockedCivics = resources.hasClass('resources-civics');
        const resourcesFloating = resources.hasClass('resources-floating');
        // Let CSS control resource panel height (avoid JS shrinking after refresh).
        ...
        if (msgQueueFloating){
            msgQueue.removeClass('right sticky');
            if (!msgQueue.hasClass('vscroll')){
                msgQueue.addClass('vscroll');
            }
            msgQueue.css('height', 0);
        }
        else if (msgQueue.hasClass('right')){
            if (msgQueue.hasClass('vscroll')){
...
function longLoop(){
    ...
    if (global.race.species !== 'protoplasm'){
        ...
        const powerGridFloatingOpen = Boolean(global.settings?.powerGridFloatingOpen);
        if (global.settings.tabLoad || (global.settings.govTabs === 2 && $(`#powerGrid`).length > 0) || (powerGridFloatingOpen && $(`#powerGridFloatingContent`).length > 0)){
            let grids = gridDefs();
...
function diffCalc(res,period){
    ...
    let el = $(`#res${res} .diff, #res${res}Outskirts .diff, #res${res}Military .diff, #res${res}Industrial .diff, #res${res}Trade .diff, #res${res}Science .diff, #res${res}Residential .diff, #res${res}Commercial .diff`);
    if (global.race['decay']){
...
function ensureCoreResourceVisibility(){
    let updated = false;
    const rawMine = global.city?.mine;
    const mineCount = Number.isFinite(rawMine?.count) ? rawMine.count : (Number.isFinite(rawMine) ? rawMine : 0);
    const rawCoalMine = global.city?.coal_mine;
    const coalMineCount = Number.isFinite(rawCoalMine?.count) ? rawCoalMine.count : (Number.isFinite(rawCoalMine) ? rawCoalMine : 0);
    const minerCap = Number.isFinite(global.civic?.miner?.max) ? global.civic.miner.max : 0;
    const coalMinerCap = Number.isFinite(global.civic?.coal_miner?.max) ? global.civic.coal_miner.max : 0;
    if (global.resource?.Knowledge && !global.resource.Knowledge.display){
        if (global.tech?.primitive >= 3 || global.tech?.science >= 1 || global.city?.university?.count > 0){
            global.resource.Knowledge.display = true;
            updated = true;
        }
    }
    if (global.resource?.Copper && !global.resource.Copper.display){
        if (mineCount > 0 || minerCap > 0){
            global.resource.Copper.display = true;
            updated = true;
        }
    }
    if (global.resource?.Coal && !global.resource.Coal.display){
        if (coalMineCount > 0 || coalMinerCap > 0){
            global.resource.Coal.display = true;
            updated = true;
        }
    }
    if (global.resource?.Money && !global.resource.Money.display){
        if (global.tech?.currency >= 1 || global.tech?.banking >= 1 || global.city?.bank?.count > 0){
