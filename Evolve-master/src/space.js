import { save, global, seededRandom, webWorker, keyMultiplier, sizeApproximation, p_on, support_on, int_on, gal_on, isMainTabActive, getMainTabIndex } from './vars.js';
...
const spaceProjects = {
    ...
    spc_dwarf: {
        ...
        shipyard: {
            ...
            sAction(){
                if (p_on['shipyard']){
                    const civicTabIndex = getMainTabIndex('civic');
                    if (civicTabIndex >= 0){
                        global.settings.civTabs = civicTabIndex;
                    }
...
const interstellarProjects = {
    ...
    int_blackhole: {
        ...
        mass_ejector: {
            ...
            special: true,
            sAction(){
                const resourcesTabIndex = getMainTabIndex('resources');
                if (resourcesTabIndex >= 0){
                    global.settings.civTabs = resourcesTabIndex;
                }
...
export function renderSpace(){
    if (!global.settings.tabLoad && !isMainTabActive('civ')){
        return;
    }
    space('inner');
    if (global.race['truepath']){
...
function space(zone){
    ...
    if (!global.settings.tabLoad){
        if (!isMainTabActive('civ') || ![1,5].includes(global.settings.spaceTabs) || (isMainTabActive('civ') && (global.settings.spaceTabs === 1 && zone !== 'inner') || (global.settings.spaceTabs === 5 && zone !== 'outer'))){
            return;
        }
    }
...
function deepSpace(){
    if (!global.settings.tabLoad && (!isMainTabActive('civ') || global.settings.spaceTabs !== 2)){
        return;
    }
...
function galaxySpace(){
    if (!global.settings.tabLoad && (!isMainTabActive('civ') || global.settings.spaceTabs !== 3)){
        return;
    }
    let parent = $('#galaxy');
    clearElement(parent);
